"""Parser for DPR all Plant.xlsx — FG Produced Qty (Pack column).

Source: ERP FG Stock/DPR all Plant.xlsx → Pivot1 sheet
Table key: dpr_production
Provides: Pack (produced qty) for R3SS
"""
...
TABLE_KEY = "dpr_production"

_SHEET_CANDIDATES = ["Pivot1", "Sheet1"]

_HEADER_RULES = [
    ("FG ITEM DESCRIPTION", "erp_code"),
    ("FG ITEM CODE", "fg_item_code"),
...
def parse(file_path):
    ...
    wb.close()
    log.info("Parsed %d rows from dpr_production %s", len(rows), file_path)
    return rows
